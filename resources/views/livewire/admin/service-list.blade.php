<div>
    <div class="flex justify-end mb-4">
        <flux:button wire:click="openCreateModal" variant="primary" size="sm">{{ __('Add Service') }}</flux:button>
    </div>
    <div class="mb-4">
        <div class="w-40">
            ...
            <table class="w-full table-auto border-collapse border border-gray-300 dark:border-gray-600 bg-white dark:bg-zinc-900">
                ...
                <tbody>
                    @foreach ($services as $service)
                        <tr wire:key="service-{{ $service->id }}" class="hover:bg-gray-50 dark:hover:bg-gray-700">
                            <td class="px-4 py-2 text-sm text-gray-900 dark:text-gray-100 border-b border-gray-200 dark:border-gray-600">{{ $service->display_order }}</td>
                            <td class="px-4 py-2 text-sm text-gray-900 dark:text-gray-100 border-b border-gray-200 dark:border-gray-600">{{ $service->name }}</td>
                            <td class="px-4 py-2 text-sm text-gray-900 dark:text-gray-100 border-b border-gray-200 dark:border-gray-600">{{ Str::limit($service->description ?? '', 50) }}</td>
                            ...
                            <td class="px-4 py-2 text-sm text-gray-900 dark:text-gray-100 border-b border-gray-200 dark:border-gray-600">
                                <flux:dropdown align="end">
                                    ...
                                    <flux:menu>
                                        <flux:menu.item wire:click="handleAction({{ $service->id }}, 'edit')">
                                            {{ __('Edit') }}
                                        </flux:menu.item>
                                        <flux:menu.item wire:click="handleAction({{ $service->id }}, 'toggle')">
                                            {{ $service->active ? __('Deactivate') : __('Activate') }}
                                        </flux:menu.item>
                                        <flux:menu.separator />
                                        <flux:menu.item wire:click="handleAction({{ $service->id }}, 'delete')">
                                            {{ __('Delete') }}
                                        </flux:menu.item>
                                    </flux:menu>
                                </flux:dropdown>
    ...
    @else
        <p class="text-gray-500 dark:text-gray-400">{{ __('No services found.') }}</p>
    @endif

    <flux:modal wire:model="showModal" class="md:w-96">
        <div class="space-y-6">
            <div>
                <flux:heading size="lg">{{ $isEditing ? __('Edit Service') : __('Add Service') }}</flux:heading>
            </div>
